    ...
    fetch(url)
        ...
        .then(async data => {
            if (data.cod === 200) {
                ...
                document.getElementById('city').textContent = `5-Day Weather Forecast of ${data.name}, ${data.sys.country}`;
                ...
                if (backgroundImageUrl) {
                    document.getElementById('weatherInfo').style.backgroundImage = `url(${backgroundImageUrl})`;
                    document.getElementById('weatherInfo').style.backgroundSize = 'cover';
                    document.getElementById('weatherInfo').style.backgroundPosition = 'center';
                } else {
                ...
                const { averageTemperatures, weatherConditions } = processWeatherData(weatherData);
                ...
                document.querySelector('.result-container').innerHTML = `<span>${errorMessage}</span>`;
...
const processWeatherData = (data) => {
    ...
    for (let date in temperatures) {
        const average = temperatures[date].reduce((a, b) => a + b, 0) / daysCount[date];
...
const renderTable = (temperatureTimeDate, pageNumber) => {
    ...
    if (filtersButton === 'Highest') {
        ...
        for (let date in temperatureTimeDate) {
            temperatureTimeDate[date].forEach(entry => {
                if (entry.condition.toLowerCase() === 'rain') {  // Check if the condition is rain
                    rainEntries.push({
                        date: date,
                        time: entry.time,
                        temp: entry.temp,
    ...
    const endIndex = Math.min(startIndex + entriesPerPage ,temperatureTimeDate2.length - 1);
    ...
    for (let i = startIndex; i < endIndex && i < temperatureTimeDate2.length; i++) {
...
function getWeatherResponse(input) {
    ...
    } else if (input.toLowerCase().includes("time of") && input.toLowerCase().includes("rain")) {
        return getTimeOfWeatherCondition(date, "Rain");
    } else if (input.toLowerCase().includes("time of") && input.toLowerCase().includes("cloud")) {
        return getTimeOfWeatherCondition(date, "Cloudy");
    } else if (input.toLowerCase().includes("time of") && input.toLowerCase().includes("drizzle")) {
        return getTimeOfWeatherCondition(date, "Drizzle");
    } else if (input.toLowerCase().includes("time of") && input.toLowerCase().includes("thunderstorm")) {
...
function getCurrentTemperature(date) {
    ...
    return weatherData && weatherData.length > 0 ? weatherData[weatherData.length - 1].temp + "°C" : "No data available.";
}
...
function getAverageTemperature(date) {
    const weatherData = temperatureTimeDate[date];
    if (!weatherData || weatherData.length === 0) return "No data available.";
    const averageTemp = weatherData.reduce((sum, data) => sum + data.temp, 0) / weatherData.length;
...
function getTimeOfWeatherCondition(date, condition) {
    ...
    const times = weatherData
        ...
        .map(data => data.time);
    return times.length > 0 ? "It will be " + condition + " at: " + times.join(", ") : "No " + condition + " data available.";
}
